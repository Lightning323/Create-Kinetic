...
public class VMCommands {
    ...
    public static void onRegisterCommands(RegisterCommandsEvent event) {
        ...
        dispatcher.register(Commands.literal("ships")
                ...
                .then(Commands.literal("deletemassless")
                        .executes(context -> {
    ...
    private static void vsCountShips(CommandContext<CommandSourceStack> cc) {
        ServerShipWorldCore shipObjectWorld =
                VSGameUtilsKt.getShipObjectWorld(cc.getSource().getServer());

        cc.getSource().sendSystemMessage(
                Component.literal("There are " + shipObjectWorld.getAllShips().size() + " total ships in the world.")
        );
    }
...
//    }
    ...
    private static int vsDeleteMasslessShips(CommandContext<CommandSourceStack> cc) {
        ...
        shipObjectWorld.getAllShips().stream().filter((s) -> {
            ...
            return s.getInertiaData().getMass() < 100;
        }).forEach((ship) -> {
        ...
        cc.getSource().sendSystemMessage(
                Component.literal("Listed " + shipsToDelete.size() + " massless ships.")
        );

        shipsToDelete.forEach(shipObjectWorld::deleteShip);

        cc.getSource().sendSystemMessage(
                Component.literal("Deleted massless ships.")
        );
